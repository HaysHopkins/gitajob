...
RSpec.describe Gitajob::Health do
  ...
  it "outputs website response to stdout", vcr: { cassette_name: "gitlab_response" } do
    expect {
      Gitajob::Health.new(website, repeat, frequency).probe
    }.to output(/https:\/\/about.gitlab.com responded with/).to_stdout
  end

  it "outputs average request time to stdout", vcr: { cassette_name: "gitlab_response" } do
    expect {
      Gitajob::Health.new(website, repeat, frequency).probe
